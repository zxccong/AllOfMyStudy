package com.zxc.eshop.cache.zk;
...
public class ZooKeeperSession {
    ...
    public ZooKeeperSession() {
        ...
        try {
            this.zookeeper = new ZooKeeper(
                    "192.168.222.131:2181,192.168.222.132:2181,192.168.222.133:2181",
                    50000,
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    count++;
                    System.out.println("this is " + count + "time try to acquire lock for product id =" + productId);
                    continue;
                }
     ...
     * @param
     */
    public void releaseFastFailedDistributedLock(String path) {
        try {
            zookeeper.delete(path, -1);

            System.out.println("release the lock for product path = " + path);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    count++;
                    System.out.println("this is " + count + "time try to acquire lock for path"+path);
                    continue;
                }
    ...
    public void releaseDistributedLock(String path) {
        try {
            ...
            System.out.println("release the lock for path"+path);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
